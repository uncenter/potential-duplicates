...
function prepare(phrase: string) {
	...
	for (const punct of PUNCTUATION) {
		ret = ret.replaceAll(new RegExp(`\\${punct}`, 'g'), ' ');
	}

	for (const word in SYNONYMS) {
		ret = ret.replaceAll(
			new RegExp((SYNONYMS)[word as keyof typeof SYNONYMS].join('|'), 'gi'),
			word,
		);
	}

	for (const exclude of EXCLUDES) {
		ret = ret.replaceAll(new RegExp(`\\b${exclude}\\s\\b`, 'g'), '');
	}
...
function similarity(a: string, b: string) {
	...
	return length === 0 ? 1 : (length - distance(a, b)) / length;
}
